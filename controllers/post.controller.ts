...
import { Tags } from "validation/tag.model";
...
export default class PostController {
  ...
  static async create(newPost: PostType, userId: number, tags: Tags) {
    await PostUpdate.parseAsync(newPost);

    await Tags.parseAsync(tags);

    return prisma.post.createPost(newPost, userId, tags);
  }
