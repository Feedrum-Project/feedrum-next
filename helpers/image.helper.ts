...
const getImageFormat = (mimetype: string) => mimetype.split("/")[1].toUpperCase() as ImageFormat

export async function sendImage(res: NextApiResponse, id: string, type: ImageFormat) {
    const image = await readFile(join(process.cwd(), `/uploads/${id}.${type.toLowerCase()}`))
...
export function parseImages(req: NextApiRequest): Promise<ImageFile[]> {
    return new Promise(async (resolve, reject) => {
        ...
        form.parse(req, (err, _fields, files) => {
            if (err) reject(err)
            else resolve(Object.values(files).map(file => {
